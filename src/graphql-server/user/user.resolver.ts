...
export class UserResolver {
  constructor(private prisma: PrismaService) {}
	...
	async login(
		...
		// getUser from name
		const user: PrismaUser = await this.prisma.user.findUnique({ where: { name: credentials.name }})
		// TODO: I have to handle the case where is not found
		const createdPassword: string = getHash({password: credentials.password, salt: user.salt})
		// compare password
	...
	async createUser(
		...
		const userAlreadyCreated = !!(await this.prisma.user.count({ where: { name: credentials.name }}))
